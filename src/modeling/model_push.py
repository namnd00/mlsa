...
def register_model(model_name, model_uri_=None, **kwargs):
    ...
    if model_uri_ is None:
        ...
        if task_instance is None:
            ValueError(
                "task_instance is required, ensure you are calling this function "
...
def register_model_by_comparison(
    data_files, registered_model_uri, model_name, push_to_production=False, **kwargs
):
    """
    ...
    metrics_registered_model, _ = evaluate_model(
        data_files, model_uri_=registered_model_uri
    )
    metrics_model_to_compare_uri, _ = evaluate_model(
        data_files, model_uri_=new_model_to_compare_uri, task_instance=task_instance
    )

    if metrics_model_to_compare_uri["val_f1"] >= metrics_registered_model["val_f1"]:
        logger.info(
            f"Model {new_model_to_compare_uri} superior to {registered_model_uri}. "
        ...
        logger.info(
            f"Model {new_model_to_compare_uri} inferior to {registered_model_uri}. "
            f"({metrics_model_to_compare_uri['val_f1']} vs {metrics_registered_model['val_f1']})"
        )
